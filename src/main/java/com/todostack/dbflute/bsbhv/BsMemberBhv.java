...
public abstract class BsMemberBhv extends AbstractBehaviorWritable<Member, MemberCB> {
     ...
     * @param memberId (会員ID): PK, ID, NotNull, BIGINT UNSIGNED(20). (NotNull)
     * @return The optional entity selected by the PK. (NotNull: if no data, empty entity)
     * @throws EntityAlreadyDeletedException When get(), required() of return value is called and the value is null, which means entity has already been deleted (not found).
     * @throws EntityDuplicatedException When the entity has been duplicated.
     * @throws SelectEntityConditionNotFoundException When the condition for selecting an entity is not found.
     */
    public OptionalEntity<Member> selectByPK(Long memberId) {
        return facadeSelectByPK(memberId);
    }

    protected OptionalEntity<Member> facadeSelectByPK(Long memberId) {
        return doSelectOptionalByPK(memberId, typeOfSelectedEntity());
    }

    protected <ENTITY extends Member> ENTITY doSelectByPK(Long memberId, Class<? extends ENTITY> tp) {
        return doSelectEntity(xprepareCBAsPK(memberId), tp);
    }

    protected <ENTITY extends Member> OptionalEntity<ENTITY> doSelectOptionalByPK(Long memberId, Class<? extends ENTITY> tp) {
        return createOptionalEntity(doSelectByPK(memberId, tp), memberId);
    }

    protected MemberCB xprepareCBAsPK(Long memberId) {
     ...
     * @param eMail (Eメール): UQ, NotNull, VARCHAR(216). (NotNull)
     * @return The optional entity selected by the unique key. (NotNull: if no data, empty entity)
     * @throws EntityAlreadyDeletedException When get(), required() of return value is called and the value is null, which means entity has already been deleted (not found).
     * @throws EntityDuplicatedException When the entity has been duplicated.
     * @throws SelectEntityConditionNotFoundException When the condition for selecting an entity is not found.
     */
    public OptionalEntity<Member> selectByUniqueOf(String eMail) {
        return facadeSelectByUniqueOf(eMail);
    }

    protected OptionalEntity<Member> facadeSelectByUniqueOf(String eMail) {
        return doSelectByUniqueOf(eMail, typeOfSelectedEntity());
    }

    protected <ENTITY extends Member> OptionalEntity<ENTITY> doSelectByUniqueOf(String eMail, Class<? extends ENTITY> tp) {
        return createOptionalEntity(doSelectEntity(xprepareCBAsUniqueOf(eMail), tp), eMail);
    }

    protected MemberCB xprepareCBAsUniqueOf(String eMail) {
        assertObjectNotNull("eMail", eMail);
        return newConditionBean().acceptUniqueOf(eMail);
    }
     ...
     * <pre>
     * <span style="color: #0000C0">memberBhv</span>.<span style="color: #CC4747">loadTask</span>(<span style="color: #553000">memberList</span>, <span style="color: #553000">taskCB</span> <span style="color: #90226C; font-weight: bold"><span style="font-size: 120%">-</span>&gt;</span> {
     ...
     */
    public NestedReferrerListGateway<Task> loadTask(List<Member> memberList, ReferrerConditionSetupper<TaskCB> refCBLambda) {
        xassLRArg(memberList, refCBLambda);
        return doLoadTask(memberList, new LoadReferrerOption<TaskCB, Task>().xinit(refCBLambda));
    }
     ...
     * (タスク)TASK by MEMBER_ID, named 'taskList'.
     * <pre>
     * <span style="color: #0000C0">memberBhv</span>.<span style="color: #CC4747">loadTask</span>(<span style="color: #553000">member</span>, <span style="color: #553000">taskCB</span> <span style="color: #90226C; font-weight: bold"><span style="font-size: 120%">-</span>&gt;</span> {
     *     <span style="color: #553000">taskCB</span>.setupSelect...
     *     <span style="color: #553000">taskCB</span>.query().set...
     *     <span style="color: #553000">taskCB</span>.query().addOrderBy...
     * }); <span style="color: #3F7E5E">// you can load nested referrer from here</span>
     ...
     * <span style="color: #3F7E5E">//    ...</span>
     * <span style="color: #3F7E5E">//});</span>
     * ... = <span style="color: #553000">member</span>.<span style="color: #CC4747">getTaskList()</span>;
     * </pre>
     * About internal policy, the value of primary key (and others too) is treated as case-insensitive. <br>
     * The condition-bean, which the set-upper provides, has settings before callback as follows:
     * <pre>
     ...
     * @return The callback interface which you can load nested referrer by calling withNestedReferrer(). (NotNull)
     */
    public NestedReferrerListGateway<Task> loadTask(Member member, ReferrerConditionSetupper<TaskCB> refCBLambda) {
        xassLRArg(member, refCBLambda);
        return doLoadTask(xnewLRLs(member), new LoadReferrerOption<TaskCB, Task>().xinit(refCBLambda));
    }

    protected NestedReferrerListGateway<Task> doLoadTask(List<Member> memberList, LoadReferrerOption<TaskCB, Task> option) {
        return helpLoadReferrerInternally(memberList, option, "taskList");
    }
     ...
     */
    public List<Long> extractMemberIdList(List<Member> memberList)
    { return helpExtractListInternally(memberList, "memberId"); }

    /**
     * Extract the value list of (single) unique key eMail.
     * @param memberList The list of member. (NotNull, EmptyAllowed)
     * @return The list of the column value. (NotNull, EmptyAllowed, NotNullElement)
     */
    public List<String> extractEMailList(List<Member> memberList)
    { return helpExtractListInternally(memberList, "EMail"); }
